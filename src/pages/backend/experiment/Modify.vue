<template>
  <el-card class="box-card">
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>编辑课程视频</el-breadcrumb-item>
    </el-breadcrumb>
...
export default {
  ...
  methods: {
    onSubmit() {
      this.$refs.dataForm.validate((valid) => {
        if (valid) {
          this.$axios.post('/manage/experiment/update', this.form).then(res => {
            if (res.success) {
              this.$message({
                message: '添加成功',
                type: 'success'
  ...
  mounted() {
    ...
    this.$axios.get('/manage/experiment/query?id=' + this.$route.query.id).then(res =>{
